...
LOCAL_TYPES = {'time', 'battery', 'version'}
...
if _ZoneInfo:
    try:
        ...
        if tz_name:
            now_local = datetime.now(tz=dt_timezone.utc).astimezone(_ZoneInfo(tz_name))
            utc_offset = int(now_local.utcoffset().total_seconds())
    except Exception:
        utc_offset = None
